...
# Receive type 11 = Pairing request ACK 
...
# Configuration send actual encoding and role
 ...
 echo "Expected $gammaCheck, got " >&2
 (
...
)|tee >(od -tx1 > mmm) | sha256sum |grep -oE '^[0-9a-fA-F]{2}' 1>&2
 ...
 (
	 ...
	 echo -n "$nonce" | xxd -r -p
 )|sha256sum |grep -oE '^[0-9a-fA-F]*' | xxd -r -p | base64)"
 jsonsize '{"status":200,"type":40, "payload": {"secret":"'$result'"}}'

) | openssl s_client -cert cert.pem -key key.pem -quiet -connect "$SERVER":6467 > result
fi

(
# Configure screen 1024x1024, one pointer, pointer mode 2
...
echo '01 02 00 10 00 00 00 00 00 00 00 00 00 00 00 01 00 00 00 04' | xxd -r -p

sleep .1

#Send KEYACTION_UP (00) for KEYCODE_HOME (3)
echo '01 02 00 10 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 04' |xxd -r -p
